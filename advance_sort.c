/*
 * ============================================================
 *  advance_sort.c  –  Advance Sorting
 *  Algoritma : Merge Sort, Quick Sort, Shell Sort
 *  Dataset   : string/kata dari file words_en.txt
 * ============================================================
 */
...
/* Shuffle array string dengan metode Fisher-Yates (rand + swap) */
...
int bacaFile(char arr[][MAX_PANJANG_KATA], int maks) {
    ...
    if (!fp) {
        ...
        printf("        Pastikan file words_en.txt ada di folder yang sama.\n\n");
        return 0;
    }
    int n = 0;
...
/*
 * MERGE SORT
 * Cara kerja : Divide and conquer. Bagi array jadi dua bagian (rekursif)
 *              hingga ukuran 1, lalu gabungkan (merge) sambil mengurutkan.
 * Kompleksitas: O(n log n) semua kasus. Stabil, butuh memori tambahan O(n).
 */
...
void merge(char arr[][MAX_PANJANG_KATA], int kiri, int tengah, int kanan) {
    ...
    /* Alokasi buffer sementara untuk sub-array kiri dan kanan */
    ...
    /* Gabungkan L dan R secara terurut kembali ke arr */
    int i = 0, j = 0, k = kiri;
    while (i < n1 && j < n2) {
        if (strcmp(L[i], R[j]) <= 0)
            strcpy(arr[k++], L[i++]);
...
void mergeSort(char arr[][MAX_PANJANG_KATA], int kiri, int kanan) {
    if (kiri < kanan) {
        int tengah = kiri + (kanan - kiri) / 2;  /* hindari overflow */
...
/*
 * QUICK SORT
 * Cara kerja : Pilih pivot (elemen terakhir), partisi array agar
 *              elemen ≤ pivot di kiri dan > pivot di kanan,
 *              lalu rekursi pada masing-masing bagian.
 * Kompleksitas: O(n log n) average, O(n²) worst. Cepat dalam praktik.
 */
...
/*
 * SHELL SORT
 * Cara kerja : Versi Insertion Sort dengan gap yang mengecil bertahap
 *              (dari n/2 terus dibagi 2 hingga gap = 1).
 *              Elemen yang jauh bisa berpindah lebih cepat.
 * Kompleksitas: O(n log² n) – lebih cepat dari Insertion Sort biasa.
 */
...
/* ----------------------------------------------------------------
 *  RUNNER – Baca file, shuffle, sort, tampilkan hasil & waktu
 * ---------------------------------------------------------------- */
